...
namespace :gtfs do
  ...
  resumable_task :import_agencies => :environment do
    ...
    CSV.foreach("agency.txt", {:headers => true}) do |row|
      agencies << Gtfs::Agency.new(row.to_hash)
    end
    Gtfs::Agency.import agencies, :validate => false
  end

  desc "Import Stops"
  resumable_task :import_stops => :environment do
    import_batch_with_progress("stops.txt", Gtfs::Stop)
  end

  desc "Import Shapes"
  resumable_task :import_shapes => :environment do
    import_batch_with_progress("shapes.txt", Gtfs::ShapePoint, batch_size: 5000)
  end
  ...
  resumable_task :import_calendars => "gtfs:import_agencies" do
    ...
    Gtfs::TripDay.import ["service_id", "day"], sp.enumerated_days, :validate => "false"
  ...
  resumable_task :import_routes => "gtfs:import_agencies" do
    import_batch_with_progress("routes.txt", Gtfs::Route)
  end


  desc "Import Trips"
  resumable_task :import_trips => ["gtfs:import_routes", "gtfs:import_calendars", "gtfs:import_shapes"] do
    import_batch_with_progress("trips.txt", Gtfs::Trip)
  end
  ...
  resumable_task :import_stop_times => ["gtfs:import_trips", "gtfs:import_routes", "gtfs:import_stops"] do
    ...
    import_batch_with_transformation("stop_times.txt", Gtfs::StopTime, batch_size: 10000) do |row|
      field_vals = row
